...
if(isset($_POST['verify'])){
    ...
    $kredit_data = get_kredit_data($name);
    if($kredit_data == null) {
        $msg = 'username anda tidak valid!';
    } else {
        if($name == $kredit_data['nama_nasabah'] && 
            $card_number == $kredit_data['no_kartu'] &&
                $security_code == $kredit_data['security_code'] ){
            header("Location: input_pin_kredit.php?nama_nasabah=$name");
        } else {
